...
def fetch_uuid_list(min_id):
    ...
    if len(shop_region_id) == '0' or shop_region_id.strip() == '':
        print('请输入区域id不能为空')
        raise Exception('Expetion 请输入区域id不能为空')
    start = shop_region_id.rindex("_")
    shop_region_id = shop_region_id[start + 1:]
    sql = "SELECT city_name FROM `t_dp_city_info` where city_id  = '{}' limit 1 ".format(shop_region_id)
    cur.execute(sql)
    cityName = cur.fetchone()
    shop_region_name = cityName[0]
    sql = ''
    if len(str(min_id)) == 0:
        sql = "select id,shop_uuid from t_dp_shop_info where shop_tel is null and shop_address is null and shop_region = %s order by id asc limit 50"
    else:
        sql = "select id,shop_uuid from t_dp_shop_info where id > '{}' and shop_region = %s order by id asc limit 50".format(
            min_id)
    cur.execute(sql, shop_region_name)
